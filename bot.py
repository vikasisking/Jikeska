...
def is_user_approved(user_id):
    try:
        with open("approval.json", "r") as f:
            ...
            return str(user_id) in approved_users.get("approved_users", [])
    except FileNotFoundError:
        logger.error("approval.json not found")
        return False
    except json.JSONDecodeError:
        logger.error("Invalid JSON in approval.json")
        return False
        ...
        for otp_entry in fetch_otp_acchubb():
            ...
            if otp_code and otp_id not in sent_ids:
                ...
                msg = (
                    ...
                    f"🔑 <b>OTP:</b> <blockquote>{html.escape(otp_code)}</blockquote>\n"
                    "━━━━━━━━━━━━━━━━━━━━━━\n"
                    "⚡️ <i>Powered by @hiden_25 Bot ❤️</i>\n"
                    "━━━━━━━━━━━━━━━━━━━━━━"
                )
                send_telegram_message(msg)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_user_approved(user_id):
        await update.message.reply_text(
            "❌ You are not approved to use this bot. Please message @hiden_25 to get approved."
        )
        return
    ...
    if action == "more_countries":
        ...
        if not carriers:
            ...
            if res.get("meta") == 200 and res.get("data"):
                data = res["data"]
                user_last_selection[user_id] = (country_id, "")
        ...
        if res.get("meta") == 200 and res.get("data"):
            data = res["data"]
            user_last_selection[user_id] = (country_id, carrier_id)
            await send_number_message(query, data, country_id, carrier_id)
        else:
            await query.edit_message_text("❌ Numbers currently not available.")

    elif action == "change_number":
        if user_id not in user_last_selection:
            await query.edit_message_text("❌ First get a number.")
            return
        country_id, carrier_id = user_last_selection[user_id]
        res = add_number(country_id, carrier_id)
        if res.get("meta") == 200 and res.get("data"):
            data = res["data"]
